...
export type ReviewedRequirements = { minAge?: number; maxAge?: number; minorRole: boolean; requiredMaterials: string[]; requiredGender?: "남성" | "여성" | null; requireCareer?: boolean; acknowledgements?: string[]; ageScope?: "source" | "pilot" };
...
export function requirementReadiness(profile: Profile | null, reviewed: ReviewedRequirements | null, materialRoles: string[] = [], now = new Date()): ReadinessIssue[] {
  ...
  if (reviewed.requiredGender && profile?.gender !== reviewed.requiredGender) issues.push({ code: "ROLE_GENDER", message: "저장한 성별이 공고의 배역 조건과 달라요. 원문 접수 방법을 확인해주세요.", target: "source" });
  if (reviewed.requireCareer && !profile?.career?.trim()) issues.push({ code: "CAREER_REQUIRED", message: "공고에서 이력을 요청했어요. 프로필에 이력을 작성하고 저장해주세요.", target: "profile" });
  const year = Number(new Intl.DateTimeFormat("en", { year: "numeric", timeZone: "Asia/Seoul" }).format(now));
  ...
  if ((reviewed.minAge != null || reviewed.maxAge != null) && (upper === null ||
    ...
    issues.push({ code: "ROLE_AGE", message: reviewed.ageScope === "pilot" ? "현재 원클릭 제공 연령 범위 밖이에요. 원문 접수 방법을 확인해주세요." : "모집 연령과 프로필을 확인해주세요. 경계 연령은 원문 확인이 필요해요.", target: "source" });
  }
