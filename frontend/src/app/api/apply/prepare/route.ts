...
export async function POST(req: Request) {
  ...
  const parsed = z.object({ auditionId: z.string().uuid(), expectedProfileVersion: z.number().int().positive(), materialIds: z.array(z.string().uuid()).max(3).default([]), ...subjectInputSchema.shape }).strict().safeParse(await req.json().catch(() => null));
  ...
  try {
    ...
    const gate = await getApplicationReadiness(db, auditionId, profile);
    if (gate.issues.length || !gate.fingerprint || !gate.subjectRules) return NextResponse.json({ error: gate.issues[0]?.message ?? "지원 조건을 다시 확인해주세요." }, { status: 409 });
    ...
    const snapshot = { ...reviewedSubject.metadata, subject: email.subject, fingerprint: gate.fingerprint, auditionId, title: audition.title, company: audition.company, recipient: email.to, replyTo: email.replyTo,
