...
export default async function UpdatesPage() {
  ...
  const [{ data: profile, error: profileError }, { data: bookmarks, error: bookmarkError }] = await Promise.all([
    ...
    db.from("bookmarks").select("audition_id").eq("user_id", user.id).order("created_at", { ascending: false }).limit(1000),
  ]);
  const filter = categoryFilter(profile?.genre ?? []);
  const recent = filter ? await db.from("public_auditions").select("id,title,deadline,category,genre")
  ...
  const { data: refs, error: refsError } = await db.rpc("owned_audition_references", { p_ids: (bookmarks ?? []).map(b => b.audition_id) });
  const closing = ((refs ?? []) as (UpdateAudition & { is_active: boolean })[]).filter(a => a.is_active && a.deadline && a.deadline >= today && a.deadline <= soon).sort((a,b) => (a.deadline ?? "").localeCompare(b.deadline ?? "")).slice(0,20);
  ...
  return <div className="space-y-6">
    ...
    <section><h2 className="mb-3 flex items-center gap-2 font-semibold"><Clock3 size={18} />3일 안에 마감하는 찜</h2>{bookmarkError || refsError ? <LoadError /> : <UpdateList items={closing} empty="곧 마감하는 찜 공고가 없어요." />}</section>
