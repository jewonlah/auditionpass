...
export function ProfilePreview({ profile, photos }: { profile: PreviewProfile; photos: string[] }) {
  ...
  const links = [["자기소개 영상", p.introduction_url], ["연기·노래·댄스 영상", p.performance_url], ["음성·보컬 샘플", p.audio_url], ["인스타그램", p.instagram_url], ["유튜브", p.youtube_url], ["포트폴리오", p.other_url]].filter(([, url]) => url);
  ...
  return <article className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm" aria-label="프로필 구성 미리보기" data-template={document.template}>
    ...
    <div className="space-y-5 px-5 pb-5">
      ...
      {!careerFirst && career}
      {p.training && <section><h3 className="mb-2 text-sm font-semibold text-gray-500">교육·트레이닝</h3><p className="whitespace-pre-line break-words text-sm leading-relaxed text-gray-700">{p.training}</p></section>}
