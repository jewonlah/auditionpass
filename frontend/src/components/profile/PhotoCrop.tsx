...
export function PhotoCrop({ file, busy, onConfirm, onCancel }: {
  ...
  const [crop, setCrop] = useState<CropChoice>({ enabled: true, x: 50, y: 50 });
  ...
  return <section className="mt-4 space-y-3 rounded-xl border border-gray-200 bg-white p-4" aria-label="사진 자르기">
    <h3 className="font-semibold">사진에 담을 부분을 골라주세요</h3>
    ...
    <div className="relative mx-auto w-full max-w-64 overflow-hidden rounded-lg bg-gray-100" style={{ aspectRatio: crop.enabled || !size.width ? "3 / 4" : `${size.width} / ${size.height}` }}>
    ...
    <p className="text-xs text-gray-500">이 구도로 새 업로드 파일을 만듭니다. 기기의 원본 파일은 유지돼요.</p>
