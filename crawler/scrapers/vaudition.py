...
class VauditionScraper(BaseScraper):
    source_name = "V오디션"
    base_url = "https://vaudition.com"
    list_url = "https://vaudition.com"
    ...
    def scrape(self) -> list[AuditionData]:
        ...
        with sync_playwright() as p:
            ...
            cards = soup.select(
                ".audition-item, .audition-card, .casting-card, "
                ".list-item, article, [class*='audition'], [class*='casting']"
            )
            if not cards:
                cards = soup.select("a[href*='audition'], a[href*='casting']")
    ...
    def _parse_card(self, card, page) -> AuditionData | None:
        ...
        # 목록/로그인 등 비상세 URL 스킵
        if any(p in href for p in ("/login", "/register", "?mode=list")):
            return None

        title_el = card.select_one(
            ".title, .casting-title, h3, h4, strong, td:nth-child(2)"
        )
        ...
        company_el = card.select_one(".company, .agency, td:nth-child(3)")
    ...
    def _fetch_detail(self, page, url: str) -> dict:
        ...
        body_el = soup.select_one(
            ".audition-detail, .casting-detail, .detail-content, "
