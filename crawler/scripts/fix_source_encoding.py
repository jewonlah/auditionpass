...
왜 중요한가: `source_name`은 단순 표시값이 아니라 **신뢰 출처 판정 키**다.
`trusted_sources`에도 손상된 이름으로 등록돼 있어, 코드만 고치면 새로 수집한 공고가
신뢰 출처에서 탈락하고 검수 큐 SAFE 판정이 안 된다. 그래서 두 테이블을 함께 고친다.
...
def main() -> int:
    ...
    plan: dict[str, str] = {}
    for name in set(broken) | set(broken_trusted):
        fixed = resolve(name)
        if fixed:
            plan[name] = fixed
        else:
    ...
    for old, new in plan.items():
        ...
        print(f"  {old!r} → {new!r} : auditions {n}건{t}")
